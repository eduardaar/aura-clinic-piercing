...
function personName(item = {}) {
  return item.name || item.client_name || item.full_name || "Cliente";
}
  ...
  return (
    <section className="premium-dashboard">
      ...
      <div className="premium-ranking-grid">
        ...
        <DashboardList title="Clientes em retorno" items={returnClients} render={(item) => `${formatDate(item.due_date)} Â· ${personName(item)} Â· ${item.reminder_day || 0} dias`} />
      </div>
    </section>
  );
}
...
function SalesWorkspace() {
  ...
  function addLineItem() {
    ...
    if (!entry) {
      setError(line.item_type === "servico" ? "Selecione um serviço antes de adicionar." : "Selecione uma joia antes de adicionar.");
      return;
    }
              ...
              <Select label="Agendamento vinculado" value={form.appointment_id} onChange={(value) => {
                ...
                {safeAppointments.map((appointment) => (
                  <option key={appointment.id} value={appointment.id}>
                    {personName(appointment)} Â· {formatDate(appointment.appointment_date)} Â· {appointment.appointment_time}
                  </option>
                ))}
              </Select>
          ...
          <div className="appointment-list">
            {safeAppointments.map((item) => (
              <article className="appointment-row" key={item.id}>
                ...
                <div><h3>{personName(item)}</h3><p>{item.procedure} Â· {currency.format(item.deposit_value || 0)} de sinal</p><small>{item.professional_name} Â· {item.whatsapp}</small></div>
