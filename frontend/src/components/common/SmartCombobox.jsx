...
export function SmartCombobox({ label, value, onChange, onSelect, options = [], placeholder = "Buscar por nome, SKU, material, cor ou medida", emptyLabel = "Nenhuma joia encontrada", required = false, loading = false, getLabel = (item) => item.name, getMeta, isDisabled = (item) => stock(item) <= 0 }) {
  const id = useId();
  ...
  function selectOnClick(event, item) {
    event.preventDefault();
    event.stopPropagation();
    // Pointer activation was already committed on pointerdown. A click with
    // detail 0 is generated by keyboard or assistive technology.
  ...
  return (
    <div className="smart-combobox" ref={root}>
      <label className="smart-combobox-label" htmlFor={id}>{label}</label>
      <div className="smart-combobox-input"><Search size={16} /><input id={id} role="combobox" aria-expanded={open} aria-controls={`${id}-list`} aria-autocomplete="list" aria-activedescendant={open && filtered[active] ? `${id}-option-${filtered[active].id}` : undefined} required={required} value={query} placeholder={placeholder} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={keyDown} />
      {loading ? <LoaderCircle className="smart-combobox-spinner" size={16} aria-label="Carregando" /> : (value || query) ? <button type="button" aria-label="Limpar seleção" onClick={() => { onChange(""); setQuery(""); setOpen(true); }}><X size={15} /></button> : <ChevronDown size={15} />}</div>
