...
// Tela independente para o menu "Contas a receber". Vendas e pagamentos
// integrados chegam pelo razão como `income`; os lançamentos criados aqui usam
// `receivable`. Os dois são receitas e precisam aparecer juntos para que a
// pessoa não precise procurar uma venda em uma tela e uma parcela manual em
// outra.
export function AccountsReceivable() {
  ...
  const initialForm = () => ({
    ...
    cost_center_id: "", installment_count: 1, notes: ""
  });
  ...
  const statusOptions = distinctOptions(entries, (item) => item.status, (value) => ENTRY_STATUS_LABELS[value] || value);
  const sourceOptions = distinctOptions(entries, (item) => item.source_type || "manual", (value) => value === "manual" ? "Lançamento manual" : value === "payment" ? "Venda ou pagamento" : value);
  ...
  function openEdit(item) {
    setForm({ ...initialForm(), ...item, due_date: String(item.due_date || "").slice(0, 10), installment_count: 1 });
    setEditing(item);
    setError("");
    setModalOpen(true);
  }
  ...
  async function registerPayment(event) {
    ...
    const response = await apiFetch(`/finance/entries/${item.id}`, {
      method: "PATCH",
      body: JSON.stringify({ paid_amount: asNumber(item.paid_amount) + asNumber(value), payment_method: item.payment_method || "Pix" })
    });
    ...
    <section className="stack">
      ...
      <section className="panel stack">
        <CrudHeader
          ...
          subtitle="Vendas integradas e lançamentos manuais"
        ...
        <DataView
          ...
          filters={[
            ...
            { key: "source_type", label: "Origem", type: "select", options: sourceOptions, match: (item, value) => (item.source_type || "manual") === value },
          ...
          columns={[
            ...
            { key: "source_type", label: "Origem", value: (item) => item.source_type || "manual", render: (item) => item.source_type === "payment" ? "Venda" : item.source_type ? financeLabel(item.source_type) : "Manual" },
            { key: "due_date", label: "Vencimento", value: (item) => String(item.due_date || "").slice(0, 10), render: (item) => formatDateWithYear(item.due_date) },
            { key: "installment", label: "Parcela", value: (item) => item.installment_number || 0, render: (item) => item.installment_count > 1 ? `${item.installment_number}/${item.installment_count}` : "À vista" },
            { key: "amount", label: "Valor", align: "right", value: (item) => asNumber(item.amount), render: (item) => currency.format(asNumber(item.amount)) },
            { key: "balance", label: "Em aberto", align: "right", value: (item) => Math.max(0, asNumber(item.amount) - asNumber(item.paid_amount)), render: (item) => currency.format(Math.max(0, asNumber(item.amount) - asNumber(item.paid_amount))) },
          ...
          actions={(item) => <RowActions actions={[
            ...
            !["paid", "canceled", "refunded"].includes(item.status) && { label: "Registrar recebimento", onClick: () => setPayment({ item, value: String(Math.max(0, asNumber(item.amount) - asNumber(item.paid_amount))) }) },
            !item.source_key && { label: "Editar", onClick: () => openEdit(item) }
          ].filter(Boolean)} />}
          empty="Nenhuma conta a receber no período."
        />
      </section>
      <Modal open={modalOpen} title={editing ? "Editar recebível" : "Novo recebível"} subtitle={editing ? "Altere apenas este lançamento." : "As parcelas são distribuídas mês a mês."} onClose={() => setModalOpen(false)}
        footer={<><Button variant="secondary" onClick={() => setModalOpen(false)}>Cancelar</Button><Button type="submit" form="receivable-form">Salvar</Button></>}>
      ...
      <Modal open={!!payment} title="Registrar recebimento" subtitle={payment?.item?.description} size="sm" onClose={() => setPayment(null)}
        footer={<><Button variant="secondary" onClick={() => setPayment(null)}>Cancelar</Button><Button type="submit" form="receivable-payment-form">Confirmar recebimento</Button></>}>
        <form id="receivable-payment-form" onSubmit={registerPayment} className="stack">
          <Input type="number" label="Valor recebido" value={payment?.value ?? ""} onChange={(value) => setPayment((current) => ({ ...current, value }))} required />
          <p className="empty-state">Em aberto: {currency.format(Math.max(0, asNumber(payment?.item?.amount) - asNumber(payment?.item?.paid_amount)))}.</p>
        </form>
      </Modal>
      <Modal open={!!details} title="Detalhes do recebível" onClose={() => setDetails(null)}>
        {details?.loading ? <Loading /> : <div className="stack">
          ...
          <div className="form-grid">
            ...
            <div><small>Origem</small><strong>{details?.source_type === "payment" ? "Venda" : details?.source_type || "Manual"}</strong></div>
            <div><small>Valor</small><strong>{currency.format(asNumber(details?.amount))}</strong></div>
            <div><small>Recebido</small><strong>{currency.format(asNumber(details?.paid_amount))}</strong></div>
            <div><small>Forma</small><strong>{details?.payment_method || "Não informada"}</strong></div>
            <div><small>Vencimento</small><strong>{formatDateWithYear(details?.due_date)}</strong></div>
          </div>
        </div>}
      </Modal>
    </section>
